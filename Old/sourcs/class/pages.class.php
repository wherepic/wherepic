...
class pages{
	...
	*/
	...
	private function setPages(){
		$pages = 1;
		//echo '$this->dataNum = '.$this->dataNum.',$this->pageNums = '.$this->pageNums;
		if($this->dataNum > $this->pageNums) { 
			...
			$multipage = ($this->currPage - $offset > 1 && $pages > $this->page ? '<a href="'.$this->mpurl.'page=1" class="first">1 ...</a>' : '').
				($this->currPage > 1 ? '<a href="'.$this->mpurl.'page='.($this->currPage - 1).'" class="prev">上一页</a>' : ''); 
				
			for($i = $from; $i <= $to; $i++) {
				$multipage .= $i == $this->currPage ? '<strong>'.$i.'</strong>' : '<a href="'.$this->mpurl.'page='.$i.'">'.$i.'</a>';
			} 
			
			$multipage .= ($to < $pages ? '<a href="'.$this->mpurl.'page='.$pages.'" class="last">... '.$pages.'</a>' : '').
				($this->currPage < $pages ? '<a href="'.$this->mpurl.'page='.($this->currPage + 1).'" class="next">下一页</a>' : '');
